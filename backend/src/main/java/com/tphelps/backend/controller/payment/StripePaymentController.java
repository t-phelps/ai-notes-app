...
import static com.tphelps.backend.service.payment.StripePaymentService.ENDPOINT_SECRET;
...
public class StripePaymentController {
    ...
    public ResponseEntity<?> createCheckoutSession(@RequestParam("lookup_key") String key){
        ...
        if(validateUserAuthentication() != null) {
            try {
                if (key == null || key.isEmpty()) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
                }

                Map<String, String> urlMap = stripePaymentService.getCreateCheckoutSessionRedirectUrl(key);
